...
rfe_resultados <- rfe(x = x_imc, 
                      ...
                      rfeControl = control_rfe, metric = "Rsquared")
...
# Recrear el modelo lineal con los predictores seleccionados para poder usar las funciones del paquete car
# Crear la fórmula del modelo con los predictores seleccionados
formula_final <- as.formula(paste("imc ~", paste(mejores_predictores_imc, collapse = " + ")))
rm <- lm(formula_final, data = muestra)
summary(rm)
```

Revisaremos como es habitual en la implementacion de estos modelos la confiabilidad de este.

recordando las condiciones que se deben cumplir para que un modelo de regresión lineal múltiple sea confiable:

1. variable de salida/respuesta continua y cuantitativa: se cumple ya que el imc es un indice numerico y continuo.

2. Los predictores deben ser cuantitativos o dicotómicos: se cumple ya que los predictores son todos cuantitativos.

3. Los predictores deben tener algún grado de variabilidad, es decir, no son constantes: se cumple ya que todos los predictores tienen varianza distinta de cero.
```{r}
for (i in mejores_predictores_imc) {
  cat("Varianza de", i, ":", var(muestra[[i]]), "\n")
}
```
como podemos observar ningun predictor observado tiene un grado de variabilidad nulo, por lo que se cumple la condicion.

ahora en el analizis se verificaran la condicion 4, 5 y 6
```{r}
# 4. Cada predictor debe estar relacionado linealmente con la respuesta.
residualPlots(rm)
marginalModelPlots(rm)
# 5. La distribución de los residuos debe ser cercana a la normal centrada en cero.
qqPlot(rm)
# 6. La variabilidad de los residuos debe ser constante (homocedasticidad).
ncvTest(rm)
...
Como se puede observar la mayoria de las variables predictoras representa una relacion lineal con la variable de respuesta, esto se reafirma con el test de cruvatura donde ninguna presento un p-valor significativo lo que indica que no hay evidencia suficiente para rechazar la hipotesis nula de linealidad. Por otra parte, el grafico Q-Q muestra que los residuos se distribuyen normalmente centrados en cero, lo que indica que la condicion 5 se cumple. Finalmente, el test de homocedasticidad indica que la variabilidad de los residuos no es constante, por lo que la sexta condicion no se cumple.

Ahora se verificara la condicion 7 la independencia de los residuos, para esto se aplicara el test de Durbin-Watson.
```{r}
durbinWatsonTest(rm)
```

se falla al rechazar la hipotesis nula de independencia de los residuos, por lo que se cumple la condicion 7.

verificando que no exista multicolinealidad entre los predictores, para esto se aplicara el test VIF.
```{r}
vif(rm)
```

los resultados obtenidos son alarmantes cuanto menos ya que un par de variables predictoras arrojaron un valor VIF mayor a 10, lo que es un valor significativo de multicolinealidad, lo que indica que el modelo no cumple con la condicion 8.

finalmente con la influencia de los valores atipicos se realizara el analisis con la funcion influencePlot del paquete car, para ver si existen valores atipicos o influyentes que puedan afectar el modelo.

```{r}
influencePlot(rm)
```

como observamos en los resultados ninguno de los valores atipicos influyen en el modelo, por lo que se cumple la condicion 9.

A modo de conclusion se puede decir que el modelo no es confiable debido a que no cumple con la condicion 6, ya que la variabilidad de los residuos no es constante, y tampoco cumple con la condicion 8, ya que existe multicolinealidad entre los predictores. Por lo tanto, se recomienda realizar una nueva seleccion de variables para el modelo, eliminando las variables que presenten multicolinealidad y realizando un nuevo analisis de confiabilidad.

a pesar de lo anterior se evaluara la calidad predictiva del modelo.
```{r}
# Prepara el data frame con los valores de RMSE
rlm2_err_df <- data.frame(RMSE = rfe_resultados[["resample"]][["RMSE"]])

# Crea el histograma usando el data frame recién creado
rlm2_err_p <- gghistogram(data = rlm2_err_df,
                          x = "RMSE",
                          bins = 5,
                          fill = "steelblue",
                          ylab = "Frecuencia",
...
cat("Rendimiento del modelo de RLM con predictores seleccionados por RFE:\n")


rendimiento_optimo <- rfe_resultados$results[rfe_resultados$results$Variables == rfe_resultados$optsize, ]
print(rendimiento_optimo, digits = 3)

cat("\nMás detalle de la raíz del error cuadrático medio (RMSE) de las remuestras:\n")
# describe() funciona bien con el data frame rlm2_err_df que contiene la columna RMSE
print(describe(rlm2_err_df, trim = 0, quant = c(0.25, 0.75),
               skew = FALSE, IQR = TRUE), digits = 3)

```
El modelo comete errores que van desde 1,006 hasta 2,084 (un RMSE promedio de 1,45±0,25). Este resultado es razonable si consideramos que la variable de respuesta (IMC) varía aproximadamente entre 16,0 y 21,6. Los errores están dentro de un rango aceptable en relación con la escala de la variable que se está prediciendo.
